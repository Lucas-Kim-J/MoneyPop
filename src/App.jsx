...
const Card = ({ children, className = '', color = 'bg-white' }) => (
  <div
    className={`border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] rounded-xl p-6 transition-transform hover:-translate-y-1 hover:shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] ${color} ${className}`}
  >
    {children}
  </div>
);
...
export default function App() {
  ...
  const handleSmartEntry = async () => {
    ...
    if (result) {
      try {
        // Remove any markdown code blocks if DeepSeek adds them
  ...
  const getStatusMessage = () => {
    if (isOverBudget)
      return { text: '警告：该吃土了！已经超支！', color: 'text-red-600', icon: <Frown size={28} /> };
    if (isWarning)
      return { text: '注意：余额告急，请理性消费。', color: 'text-orange-600', icon: <AlertTriangle size={28} /> };
          ...
          >
            ...
            {isEditingBudget ? (
              <div className="flex gap-2 mb-4 relative z-10">
                ...
                />
                <Button onClick={handleUpdateBudget} className="py-2 px-3" variant="black">
                  OK
                </Button>
              </div>
            ) : (
            ...
            <Card color="bg-green-400" className="flex-1 flex flex-col justify-center">
              ...
              <div className="flex justify-between items-end">
                <div>
                  ...
                  <p className="text-2xl font-black text-white drop-shadow-[2px_2px_0px_rgba(0,0,0,0.5)]">
                    +¥{totalIncome}
                  </p>
                </div>
            ...
            <Card className="sticky top-24 transition-colors duration-300" color={type === 'expense' ? 'bg-white' : 'bg-green-50'}>
              ...
              {showSmartInput ? (
                ...
                <div className="space-y-4 animate-in fade-in zoom-in-95">
                  ...
                  <Button onClick={handleSmartEntry} variant="magic" className="w-full" disabled={isAiProcessing}>
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="font-bold text-lg flex items-center gap-2">
                              ...
                              {!isExp && (
                                <span className="text-xs bg-green-100 text-green-700 px-1 border border-green-300 rounded">
                                  收入
                                </span>
                              )}
                            </div>
